...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['mark_paid'])) {
        ...
        $selectSql = "SELECT * FROM booking WHERE id = $bookingId";
        $result = $conn->query($selectSql);

        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $deletedSeats = $row['num_seats_booked'];
            ...
            $updateSql = "UPDATE booking SET total_price = total_price - ($deletedSeats * seat_price) WHERE booking_date='$booking_date' AND booking_time='$booking_time'";
            $conn->query($updateSql);
        }
    }
...
$sql = "SELECT booking.*, movie.name AS movie_name, GROUP_CONCAT(seat_num ORDER BY seat_num) AS seat_numbers, COUNT(*) AS num_seats_booked, user.name AS user_name
        ...
        ORDER BY booking_date DESC, booking_time DESC"; // Added ORDER BY clause
